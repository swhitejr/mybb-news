...
class Shinka_QueryBuilder_Adapter
{
    ...
    private function getUpdateStatement($data)
    {
        ...
        // foreach ($data as $key => $value) {
        //     if ($value instanceof Shinka_Raw) {
    ...
    protected function buildCriteria($statements, $bindValues = true)
    {
        ...
        foreach ($statements as $statement) {
            ...
            if (is_null($value) && $key instanceof \Closure) {
            ...
            } elseif ($value instanceof Shinka_QueryBuilder_Raw) {
                ...
                if (!$bindValues) {
                    ...
                    $criteria .= $statement['joiner'] . ' ' . $key . ' ' . $statement['operator'] . ' ' . $value . ' ';
                    // } elseif ($statement['key'] instanceof Shinka_QueryBuilder_Raw) {
    ...
    public function wrapSanitizer($value)
    {
        ...
        // if ($value instanceof Shinka_QueryBuilder_Raw) {
        //     return (string) $value;
    ...
    protected function buildJoin($statements)
    {
        ...
        foreach ($statements['joins'] as $joinArr) {
            if (is_array($joinArr['table'])) {
                ...
                // $table = $joinArr['table'] instanceof Shinka_QueryBuilder_Raw ?
            ...
            $sqlArr = array(
                ...
                $table,
                'ON',
                $joinBuilder->getQuery('criteriaOnly', false),
            );
